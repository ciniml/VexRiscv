...
fn main() {
    ...
    let svd = std::fs::read_to_string("vexriscv-vitis.svd").expect("Failed to read SVD file");
    ...
    std::fs::write("src/lib.rs", &output_string).expect("Failed to write pac/lib.rs");

    println!("cargo:rerun-if-changed=build.rs");
    println!("cargo:rerun-if-changed=vexriscv-vitis.svd");
}
